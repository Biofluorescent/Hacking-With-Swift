...
//TODO: - Refactor loadLevel() into smaller methods
//      - Implement what happens when hitting finish flag. (load another level)
//      - Possibly create new obstacle... teleporter etc..
...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    //MARK: - Touch functions for simulator use
    ...
    func loadLevel() {
        if let levelPath = Bundle.main.path(forResource: "level1", ofType: "txt") {
            if let levelString = try? String(contentsOfFile: levelPath) {
                ...
                //Read in reverse since SpriteKit Y:0 is at bottom of screen
                for (row, line) in lines.reversed().enumerated() {
                    for (column, letter) in line.enumerated() {
                        //Add 32, SpriteKit calculates position at object center
