...
class ResultsViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        //Get all suggestions for the whistle
    ...
    func parseResults(records: [CKRecord]) {
        ...
        //Pull out text for each suggestion
        for record in records {
            newSuggestions.append(record["text"] as! String)
        }
        
        //Never modify data being used on main thread in background thread
        ...
        CKContainer.default().publicCloudDatabase.fetch(withRecordID: whistle.recordID) { [unowned self] record, error in
            if let error = error {
                DispatchQueue.main.async {
                    let ac = UIAlertController(title: "Fetch Failed", message: "There was a problem fetching the whistle; please try again: \(error.localizedDescription)", preferredStyle: .alert)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            //user's comments about this whistle
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        //Exit if row tapped was not the last one
